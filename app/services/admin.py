...
from ..models import AdminMember, BotSettings, ManualTopUpStatus, User
...
class AdminService:
    ...
    async def get_bot_settings(self) -> BotSettings:
        async with session_scope() as session:
            ...
            return await settings_repo.get()

    async def update_greeting_text(self, text: str | None) -> BotSettings:
        async with session_scope() as session:
            settings_repo = BotSettingsRepository(session)
    ...
    async def update_greeting_image(self, file_id: str | None) -> BotSettings:
        async with session_scope() as session:
            settings_repo = BotSettingsRepository(session)
            settings = await settings_repo.get()
            settings.greeting_image_file_id = file_id
            await session.flush()
            return settings
