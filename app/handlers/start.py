...
async def _get_home_content(admin_service: AdminService) -> tuple[str, str | None]:
    settings = await admin_service.get_bot_settings()
    text = settings.greeting_text or _home_text()
    return text, settings.greeting_image_file_id
...
async def cmd_start(
    ...
    text, photo = await _get_home_content(admin_service)
    await _respond(message, text=text, markup=portal_main_keyboard(), photo=photo)
...
async def portal_home(
    ...
    await state.clear()
    text, _ = await _get_home_content(admin_service)
    await _respond(
        callback.message,
        text=text,
        markup=portal_main_keyboard(),
        edit=True,
    )
